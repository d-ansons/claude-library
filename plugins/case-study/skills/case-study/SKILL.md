...
6. **Update the draft file with the new answers** — fill in the real content and delete the resolved Follow-up Questions items (see Tracking Follow-up Questions below). **Then explicitly re-check the full completeness checklist against the whole conversation so far, not just against what round 1 covered** — this includes items you deliberately deferred out of round 1 (Depth, Reflection, CV targeting, Parent Engagement, Timeline sequencing). Add whatever's still missing as new Follow-up Questions items and ask the next batch of 3-5. **One round of questions is essentially never the whole loop — treat "I got answers back" as the signal to check what's next, not as the signal to wrap up.** Keep looping — ask, update, re-check — until the Follow-up Questions section is empty and every checklist item is either filled or explicitly marked not applicable/not known (because you asked and the user said so). Most case studies take 2-4 rounds to get there — if you're well past that, you're probably over-interrogating; ask what's left in one final pass.

7. **Never fabricate — and never mark something "Not specified" without having actually asked about it first.** Do not invent metrics, outcomes, tools, team sizes, or timeframes the user hasn't stated. "Not specified" is only for a checklist item you asked about and got no answer to (or the user said they don't know/it doesn't apply) — using it to quietly skip a question you never asked is the same failure as fabricating an answer, just in the other direction.
...
- Whenever you send a batch of questions in chat, the file's items must match them exactly — write to the file first (or in the same turn), never leave the file behind what you just asked in chat.
- When an answer comes in, fill in the real content in its section and **delete** the corresponding item from the list — don't just check it off and leave it. A shrinking list makes it unambiguous what's still open; a list full of checked-off items is easy to mistake for "done enough" and stop looping prematurely.
- If an answer surfaces a new gap (including a depth-probing or reflection follow-on), add it as a new item immediately, don't just hold it in memory for the next round.
- The section is empty exactly when there's nothing left to ask — that's the real signal to move to step 8, not "we've done a round or two."
- Once the case study is complete (step 9), delete the whole section (it'll already be empty) — it's drafting scaffolding, and the finished case study shouldn't carry it.
...
- Never ask a question in chat that isn't also sitting as an item in the file's Follow-up Questions section, and never resolve an item without also filling in the real content in its section — the two happen together, not one before the other.
- Small batches beat one big dump: 3-5 questions per round keeps the user able to actually answer everything, even if it takes one or two more rounds than firing off every gap at once would. But more rounds is the point, not a failure mode — a case study answered in a single round almost certainly skipped Depth, Reflection, or CV targeting rather than actually being done. Keep asking until Follow-up Questions is genuinely empty.
